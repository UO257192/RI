...
public class EnrollmentJpaRepository extends BaseJpaRepository<Enrollment> implements EnrollmentRepository {
	...
	public Optional<Enrollment> findByMechanicCourse(Course course, Mechanic mechanic) {
		return Jpa.getManager().createNamedQuery("Enrollment.findByMechanicCourse", Enrollment.class)
				.setParameter(1, course).setParameter(2, mechanic).getResultList().stream().findFirst();
	}

	@Override
	public List<Enrollment> findAttendanceByCourseId(Course course) {
		return Jpa.getManager().createNamedQuery("Enrollment.findAttendanceByCourseId", Enrollment.class)
