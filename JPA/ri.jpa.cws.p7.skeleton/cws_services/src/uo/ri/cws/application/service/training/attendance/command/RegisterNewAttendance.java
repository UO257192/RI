...
public class RegisterNewAttendance implements Command<EnrollmentDto> {
    ...
    public EnrollmentDto execute() throws BusinessException {
        ...
        BusinessCheck.isTrue(!enrollmentRepository.findByMechanicCourse(course, mechanic).isPresent(), "Ya existe una asistencia registrada para el mecanico y el curso");
        BusinessCheck.isTrue(enrollmentDto.passed && !(enrollmentDto.attendance < 85), "El mecanico no puebe aprobar un curso sin tener el 85% de asistencia");
        BusinessCheck.isTrue(enrollmentDto.attendance <= 100 && enrollmentDto.attendance >= 0 , "El porcentaje de asistencia no está entre 0 y 100");
